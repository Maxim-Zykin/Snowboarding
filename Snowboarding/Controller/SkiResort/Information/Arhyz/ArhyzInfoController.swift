...
class ArhyzInfoController: UIViewController {

    // MARK: - UI Components
    private let aboutResort = CustomLabel(text: "Всесезонный курорт в горах Северного Кавказа, ориентирован на активный семейный отдых. Зимой сюда едут кататься на лыжах и сноуборде, летом доступны трассы для горного велосипеда, пешие и конные маршруты, джиппинг, экскурсии по красивейшим окрестностям Архыза.",  textAlignment: .left, size: 18, color: .black, numberOfLines: 0)
    ...
    private let backgroundViewWeather: UIView = {
        let view = UIView()
        view.backgroundColor = UIColor(red: 72/255, green: 150/255, blue: 192/255, alpha: 255/255)
        view.layer.cornerRadius = 35
        return view
    }()

    private var imageArhyz = UIImageView()
    
    private var infoModel = InfoGroupArhyz.skiInfi()
    ...
    let weather = CustomLabel(text: "Сейчас в Архызе",size: 20, color: .white)
    ...
    func tempFeach() {
        NetworkManager.fetchWeather(url: API.apiWeatherArhyz) { [weak self] fields, icon in
    ...
    private func setupUI() {
        ...
        self.view.addSubview(imageArhyz)
        ...
        self.imageArhyz.translatesAutoresizingMaskIntoConstraints = false
        ...
        NSLayoutConstraint.activate([
            ...
            imageArhyz.topAnchor.constraint(equalTo: self.contentView.topAnchor),
            imageArhyz.leadingAnchor.constraint(equalTo: self.contentView.leadingAnchor),
            imageArhyz.trailingAnchor.constraint(equalTo: self.contentView.trailingAnchor),
            imageArhyz.heightAnchor.constraint(equalToConstant: 300),
            imageArhyz.widthAnchor.constraint(equalTo: self.contentView.widthAnchor),
            ...
            backgroundViewWeather.topAnchor.constraint(equalTo: self.imageArhyz.bottomAnchor, constant: -40),
...
extension ArhyzInfoController: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        case .hotels:
            ...
            vc.model = hotelsArhyzAll
            self.navigationController?.pushViewController(vc, animated: true)
        case .rent:
            let vc = RentViewController()
            vc.model = rentsArhyzAll
            self.navigationController?.pushViewController(vc, animated: true)
        case .map:
            let vc = MapViewController()
            vc.latitude = 43.544585
            vc.longitude = 41.181382
            self.navigationController?.pushViewController(vc, animated: true)
        }
    }
}
