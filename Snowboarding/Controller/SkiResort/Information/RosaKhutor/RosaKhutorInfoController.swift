...
class RosaKhutorInfoController: UIViewController {
    ...
    // MARK: - UI Components
    private let aboutResort = CustomLabel(text: "Роза Хутор — круглогодичный горный курорт, расположенный на берегах реки Мзымта и горных склонах к югу от неё в Адлерском районе Сочи.",  textAlignment: .left, size: 18, color: .black, numberOfLines: 0)
    ...
    private let backgroundViewWeather: UIView = {
        let view = UIView()
        view.backgroundColor = UIColor(red: 38/255, green: 74/255, blue: 121/255, alpha: 255/255)
    ...
    private var infoModel = InfoGroupRosaKhutor.skiInfi()
    ...
    let weather = CustomLabel(text: "Сейчас в Роза Хутор",size: 20, color: .white)
    ...
    func tempFeach() {
        NetworkManager.fetchWeather(url: API.apiWeatherRosaKhutor) { [weak self] fields, icon in
...
extension RosaKhutorInfoController: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        case .map:
            ...
            vc.latitude = 43.646468
            vc.longitude = 40.331964
            self.navigationController?.pushViewController(vc, animated: true)
        }
    }
}
